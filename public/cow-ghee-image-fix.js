(function(){
  var NEW_IMAGE='/products/cow ghee batti 2.png';
  var HERO_IMAGE='/visuals/hero-new.svg';
  var HERO_REMOTE='plain-apac-prod.komododecks.com/202609/06/FdqS848cNUjuemA1XN0k/image.jpg';
  function fix(){
    document.querySelectorAll('img').forEach(function(img){
      var src=img.getAttribute('src')||'';
      var low=src.toLowerCase();
      if(low.indexOf('cow ghee batti')!==-1 && src!==NEW_IMAGE){img.setAttribute('src',NEW_IMAGE);return;}
      if(low.indexOf(HERO_REMOTE)!==-1 && src!==HERO_IMAGE){img.setAttribute('src',HERO_IMAGE);}
    });
  }
  function biggerText(){
    if(document.getElementById('pp-bigger-text')) return;
    var s=document.createElement('style');s.id='pp-bigger-text';
  ...
  function boot(){fix();biggerText();}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();
  var timer=setInterval(fix,500);setTimeout(function(){clearInterval(timer)},8000);
})();