...
export default function CustomizeKit() {
  ...
  const add = (name) => setSelected((current) => ({ ...current, [name]: (current[name] || 0) + 1 }));
  const remove = (name) => setSelected((current) => ({ ...current, [name]: Math.max(0, (current[name] || 0) - 1) }));

  return (
    <div className="custom-kit-page">
      <header>
        <a className="brand" href="/#home"><div className="logo-mark"><span>ॐ</span><b>PP</b></div><div><b>POOJAN</b><em>PARADISE</em><small>Shuddh Samagri • Shreshth Seva</small></div></a>
      </header>
      <main className="custom-kit">
        ...
        <div className="kit-price-card">
          ...
          <b>{total < PRICE ? `₹${remaining} more required to complete your kit` : over > 0 ? `₹${over} over ₹1,299 — remove or reduce items` : '✓ ₹1,299 kit is complete and ready to buy'}</b>
        </div>
        <section className="custom-items">
          {ITEMS.map(([name, price]) => (
            <article key={name} className={selected[name] ? 'chosen' : ''}>
              <div><h3>{name}</h3><span>₹{price} each</span></div>
              <div className="item-controls"><button type="button" onClick={() => remove(name)} disabled={!selected[name]}>−</button><b>{selected[name] || 0}</b><button type="button" onClick={() => add(name)}>+</button></div>
            </article>
          ))}
        </section>
        <div className="custom-kit-bottom">
          <div><span>Selected Kit Value</span><strong>₹{total.toLocaleString('en-IN')}</strong></div>
          <button type="button" disabled={!canOrder}>{canOrder ? 'Add Customized Kit to Cart' : total < PRICE ? `Add ₹${remaining} More` : `Reduce ₹${over}`}</button>
        </div>
      </main>
    </div>
  );
